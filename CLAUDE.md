...
### Cartes
- `CardData` (ScriptableObject, `Cards/CardData.cs`, gros fichier) est **data-driven** : dégâts, ciblage (`CardTargetType`, `CardAreaEffect`), type de dégâts (`DamageType`), poussée/tirage, charge (ligne droite, `isChargeCard`), bond (saut sur une case vide puis effet depuis l'arrivée, `leapToTarget`), catégorie de slot (`CardCategory` : Signature / Standard / Awakening = Éveil). La résolution passe par `CardData.ExecuteEffect(source, target, tile, isAdditionalMultiTargetHit)`, appelée par `HandUIController` (joueur) et `EnemyAI`. Une nouvelle carte = en général un nouvel asset, pas une nouvelle classe ; ajouter un champ/enum seulement si l'effet n'est pas exprimable.
