...
### Combat
- `TurnStateMachine` (classe C# pure, pas MonoBehaviour) porte l'état du tour ; `GridManager` orchestre la grille (**carrée** 10×10, positions `Vector2Int`, **8 directions** : une diagonale vaut 1 case — toute distance, voisinage ou direction passe par `GridGeometry`, ne pas recalculer à la main), le spawn et la rotation des tours (un tour par unité dans l'ordre de `_units`, invocations sautées), avec `GridRepository` pour les données de grille.
