...
### Combat
- `TurnStateMachine` (classe C# pure, pas MonoBehaviour) porte l'état du tour ; `GridManager` orchestre la grille (**carrée** 10×10, positions `Vector2Int`, **4 directions**, distance de Manhattan — toute distance, voisinage ou direction passe par `GridGeometry`, ne pas recalculer à la main, écho du Miroir fraternel compris), le spawn et la rotation des tours (un tour par unité dans l'ordre de `_units`, invocations sautées), avec `GridRepository` pour les données de grille.
